...
llama_model::llama_model(const llama_model_params & params) : params(params), pimpl(std::make_unique<impl>()) {
    ...
    // prima: copy layer filter from params to model members
    layer_load_filter = params.layer_load_filter;
    layer_map_fn      = params.layer_map;
    layer_filter_ud   = params.layer_filter_user_data;

    // prima: copy layer device select callback
    layer_dev_select    = params.layer_dev_select;
    layer_dev_select_ud = params.layer_dev_select_ud;
}
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    auto get_layer_buft_list = [&](int il) -> llama_model::impl::layer_dev {
        // prima: layer device select callback for distributed CPU/GPU hybrid
        if (layer_dev_select) {
            ggml_backend_dev_t dev = layer_dev_select(il, layer_dev_select_ud);
            if (dev != nullptr) {
                // check if this is a known GPU device
                auto gpu_it = pimpl->gpu_buft_list.find(dev);
                if (gpu_it != pimpl->gpu_buft_list.end()) {
                    LLAMA_LOG_DEBUG("load_tensors: layer %3d -> %s (layer_dev_select: GPU)\n", il, ggml_backend_dev_name(dev));
                    return {dev, &gpu_it->second};
                }
                // not a GPU device (or unknown device) — use CPU
                LLAMA_LOG_DEBUG("load_tensors: layer %3d -> %s (layer_dev_select: CPU)\n", il, ggml_backend_dev_name(cpu_dev));
                return {cpu_dev, &pimpl->cpu_buft_list};
            }
            // nullptr return means "use default logic"
        }
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            // prima: skip tensors rejected by load filter
            // tn.bid >= 0 for per-layer tensors, -1 for global tensors (tok_embd, output_norm, output)
            // the filter callback decides for both cases
            if (layer_load_filter && !layer_load_filter(tn.bid, layer_filter_ud)) {
                // TENSOR_DUPLICATED references an already-counted tensor — don't
                // adjust counters (matches ml.create_tensor() which only bumps
                // size_data, not n_created, for duplicates).
                if (!(flags & TENSOR_DUPLICATED)) {
                    ggml_tensor * t_skip = ml.get_tensor_meta(tn.str().c_str());
                    if (t_skip) {
                        ml.size_data -= ggml_nbytes(t_skip);
                        ml.n_created++;
                    }
                }
                return nullptr;
    ...
    // When a layer load filter is active (distributed inference), disable prefetch
    // to avoid MAP_POPULATE loading the entire file into physical memory.
    // Only the pages for actually-used tensors will be demand-paged.
    const bool do_prefetch = (layer_load_filter == nullptr);
...
llama_memory_i * llama_model::create_memory(const llama_memory_params & params, const llama_cparams & cparams) const {
    ...
    // wrap C-style layer filter callback into std::function for KV cache constructors
    llama_memory_i::layer_filter_cb filter = nullptr;
    if (params.layer_filter) {
        auto cb = params.layer_filter;
        auto ud = params.layer_filter_ud;
        filter = [cb, ud](int32_t il) { return cb(il, ud); };
    }
        ...
        default:
            {
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    // prima: compose user's distributed layer filter with
                    // the architecture type-based filters so that hybrid
                    // memory only allocates for layers owned by this device.
                    if (filter) {
                        const auto * hp = &hparams;
                        if (filter_attn) {
                            auto orig = std::move(filter_attn);
                            filter_attn = [orig = std::move(orig), filter](int32_t il) {
                                return orig(il) && filter(il);
                            };
                        } else {
                            filter_attn = [filter, hp](int32_t il) {
                                return !hp->is_recurrent(il) && filter(il);
                            };
                        }
                        if (filter_recr) {
                            auto orig = std::move(filter_recr);
                            filter_recr = [orig = std::move(orig), filter](int32_t il) {
                                return orig(il) && filter(il);
                            };
                        } else {
                            filter_recr = [filter, hp](int32_t il) {
                                return hp->is_recurrent(il) && filter(il);
                            };
                        }
                    }
...
llama_model_params llama_model_default_params() {
    llama_model_params result = {
        ...
        /*.tensor_source_overrides     =*/ nullptr,
        /*.n_gpu_layers                =*/ -1,
        ...
        /*.layer_load_filter           =*/ nullptr,
        /*.layer_map                   =*/ nullptr,
        /*.layer_filter_user_data      =*/ nullptr,
        /*.layer_dev_select            =*/ nullptr,
        /*.layer_dev_select_ud         =*/ nullptr,
        /*.vocab_only                  =*/ false,
        /*.use_mmap                    =*/ true,
        /*.use_direct_io               =*/ false,
        /*.use_mlock                   =*/ false,
...
void llama_model_set_layer_filter(
        llama_model *              model,
        llama_layer_load_filter_cb load_filter,
        llama_layer_map_cb         layer_map,
        void * user_data) {
    model->layer_load_filter = load_filter;
    model->layer_map_fn      = layer_map;
    model->layer_filter_ud   = user_data;
}
